...
|----|--------|------------------------|
| `react/no-fetch-in-useeffect` | Do not kick off HTTP data loading inside `useEffect` / `useLayoutEffect` | `error` |
| `react/query-error-handled` | TanStack `useQuery` / `useInfiniteQuery` must handle errors (structural R1-lite) | `error` |
